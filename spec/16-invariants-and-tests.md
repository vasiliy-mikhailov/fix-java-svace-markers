...
```
agent/src/test/java/tech/mikhailov/fsm/agent/*.java   44 JUnit 5 classes
...
| `OneOrderNotThreeTest.triples` | the names now DO follow a rule — `<stage>-<role>` — so deriving the three from the stage would pass on a stage that had quietly lost one of them, which is the thing being caught | the five triples are written out literally, and every name in `CHAIN` is separately required to end `-planner`, `-doer` or `-verifier` |
...
|---|---|---|
| `ADeclarationNotAMentionTest` | `Prove.verdict(reply, allowed…)` returns the **last declared** verdict; a heading, colon, backticks and `**` are decoration; with nothing declared it falls back to the earliest mention | the propose-doer on `PasswordResetLink:21` declared `**reject**` on its last line; reading the earliest mention took "make" out of *"makes admin reset links predictable"* and settled the marker pr-ready — a patch that breaks a graded lesson, against the explicit judgement of both agents that exist to stop that |
| `RoutingAWordTest` | a judge's own explanation must not overrule its verdict; `rejects("")` and `rejects(unreadable)` are **true**; a word nobody allowed yields `""` | the fix-verifier led with `sound` and later wrote "over-fit" while explaining what the patch was *not*; a substring search read a careful acquittal as a conviction and the chain retried a fix nobody had faulted |
| `SilenceIsNotADecisionTest` | `Prove.declined(reply)` is false for `null`, `""` and whitespace; true for the token `no test`, plain or decorated; false for prose that merely reasons towards no defect | in a 67-marker run the reproduce-doer returned nothing 53 times out of 133, and every one was passed to the argue-doer as *"the reproduce-doer declined to write a test, saying:"* followed by nothing. Four markers settled on that |
...
| `OneOrderNotThreeTest` | there is **one** agent order: `ORDER = CHAIN + WATCH + ASKED` in that order, `ORDER.size() == Agents.builtIn(...).size()`, no name appears twice, and `builtIn` returns its keys in `ORDER`'s sequence and not the hash's. `CHAIN` is fifteen names read three at a time as five planner/doer/verifier triples — `reproduce-planner`/`reproduce-doer`/`reproduce-verifier`, then `fix-`, `propose-`, `argue-`, `price-` — in the order `Prove` calls them, with `CHAIN.size() % 3 == 0` so no stage may be missing a role and every name required to end `-planner`, `-doer` or `-verifier`. `WATCH` is `overwatch-planner`, `overwatch-doer`, `overwatch-verifier`, `interpreter-planner`, `interpreter-doer`, `interpreter-verifier`; `ASKED` is `chat` alone. `argue-verifier` is in `CHAIN`; no `WATCH` name is, and no `ASKED` name is in either | the marker tabs held their own list and it was missing `argue-verifier` — so the agent that can send a settlement back for rework had no page, and its answers could be read only by scrolling the whole trace. Nothing failed; a tab was simply never there. An agent missing from the order still appears, at the end, sorted, which is the drift this replaced rather than a fix for it. The names in `CHAIN` are written out rather than derived for the same reason: the record from before the roles became three carries the old ones — `reproducer`, `fixer`, `pr-maker`, `verdict`, `estimator` and their critics — and the marker page appends any agent it does not recognise, so a run proved under the old names keeps its tabs |
...
| a verifier that throws | the doer's answer **stands**; an objection must be raised to bite | `TheVerdictAnswersToSomebodyTest.silenceWaives` |
...
| a verifier that omits the `SHORT:` label | the first sentence becomes the short form; both halves survive | `ALaneIsTheUnitNobodySeesTest.noLabel` |
