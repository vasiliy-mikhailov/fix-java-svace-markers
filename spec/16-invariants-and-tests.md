...
| `OneOrderNotThreeTest.pairs` | deriving each critic's name from its producer's would test a convention this code does not have (`fix-doer`/`fix-verifier`, `reproduce-doer`/`reproduce-verifier`) | the five pairs are written out literally |
...
| `SilenceIsNotADecisionTest` | `Prove.declined(reply)` is false for `null`, `""` and whitespace; true for the token `no test`, plain or decorated; false for prose that merely reasons towards no defect | in a 67-marker run the reproduce-doer returned nothing 53 times out of 133, and every one was passed to the verdict agent as *"the reproduce-doer declined to write a test, saying:"* followed by nothing. Four markers settled on that |
