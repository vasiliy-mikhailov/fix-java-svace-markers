...
n8n is gone from the deployment. `n8n/docker-compose.yml` declares three services — `engine`,
...
`n8n/docker-compose.yml` binds the repository at `/data` **read-only** and adds a second, **writable**
...
`cd n8n-fleet && mvn -B test` runs both modules. Nothing needs a network, a container or a model.
...
- **Not a compose service.** `n8n/docker-compose.yml` declares exactly three services and
