...
**THE CUTOVER IS DONE.** `n8n/docker-compose.yml` declares three services — `engine`, `orchestrator`,
...
**The build context is the reactor root (`n8n-fleet/`), not this directory.** `runner` depends on
...
DOCKER_BUILDKIT=0 docker build --network mvn-cache -f runner/Dockerfile \
  ...
  -t fsm-runner:latest .
docker compose -f n8n/docker-compose.yml up -d runner    # uses the tag; does not rebuild
```
...
- **All four module poms are copied**, including `orchestrator/pom.xml`, which this image never compiles.
  ...
  A module added to `n8n-fleet/pom.xml` is a `COPY` line added to `runner/Dockerfile` and to
...
### 4. Point n8n at it — code change, not a compose change

```bash
cd n8n-fleet/n8n/agentic
# src/gen-prover.js, line ~31:
#   -const RUNNER = 'http://fsm-java-runner:8090';
#   +const RUNNER = 'http://fsm-runner:8090';
node src/gen-prover.js                                 # rewrites workflow_prover.json
grep -c 'fsm-java-runner:8090' workflow_prover.json    # must be 0 (it is 6 before the edit)
grep -c 'fsm-runner:8090'      workflow_prover.json    # must be 6
```

Then deploy the workflow. If you generated it anywhere other than the deployment host, **rsync it first**:
`deploy.sh` imports the container's copy of the file (compose mounts the repo root at `/data`), so a
regenerated artifact that was never copied over deploys the old URL and its own pre-flight passes on the
stale file. `deploy.sh` then imports, **re-activates** (import silently deactivates what it overwrites),
restarts `fsm-n8n` and waits:

```bash
./deploy.sh workflow_prover.json
```

Its post-flight checks the *engine's* routes, not the runner's, so verify this one by hand — from inside
the n8n container, which is where the workflow's HTTP nodes run:

```bash
docker exec fsm-n8n node -e \
  'fetch("http://fsm-runner:8090/health").then(r=>r.text()).then(console.log).catch(e=>{console.error(e.message);process.exit(1)})'
```
...
— `mvn -pl orchestrator test -Dtest=DeploymentTest` from `n8n-fleet/`. That is where "the runner must be
